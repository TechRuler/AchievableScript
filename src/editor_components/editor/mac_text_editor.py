...
class SyntaxHighlighter:
    def __init__(self, master):
        ...
        self.master.bind("<ButtonRelease-1>", self._on_key_release)
    ...
    def _setup_tags(self):
        for tag, color in self.colors.items():
            self.master.tag_configure(tag, foreground=color)
    ...
    def _highlight(self):
        ...
        self.master.tag_remove("keyword", "1.0", "end")
        self.master.tag_remove("string", "1.0", "end")
        self.master.tag_remove("comment", "1.0", "end")
        self.master.tag_remove("builtin", "1.0", "end")
        self.master.tag_remove("number", "1.0", "end")

        for index, token, value in self._tokenize(code):
            if token == "keyword":
                self._apply_tag("keyword", index, value)
            elif token == "string":
                self._apply_tag("string", index, value)
            elif token == "comment":
                self._apply_tag("comment", index, value)
            elif token == "builtin":
                self._apply_tag("builtin", index, value)
            elif token == "number":
                self._apply_tag("number", index, value)

    def _apply_tag(self, tag, index, value):
        start = f"{index[0]}.{index[1]}"
        end = f"{index[0]}.{index[1] + len(value)}"
    ...
    def _tokenize(self, code):
        ...
        for tok in tokenize.generate_tokens(reader):
            ...
            if token_type == "NAME" and tok.string in self.keywords:
                tokens.append(((tok.start[0], tok.start[1]), "keyword", tok.string))
            elif token_type == "STRING":
                tokens.append(((tok.start[0], tok.start[1]), "string", tok.string))
            elif token_type == "COMMENT":
                tokens.append(((tok.start[0], tok.start[1]), "comment", tok.string))
            elif token_type == "NUMBER":
                tokens.append(((tok.start[0], tok.start[1]), "number", tok.string))
            elif token_type == "NAME" and tok.string in dir(__builtins__):
                tokens.append(((tok.start[0], tok.start[1]), "builtin", tok.string))
...
class Editor(tk.Frame):
    def __init__(self, *arg, **kwarg):
        ...
        self.editor = tk.Text(self, font=("Monaco", 15), wrap="none")
